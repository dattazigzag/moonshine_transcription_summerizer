...
# Also write a PDF alongside the markdown (opt-in, off by default):
uv run main.py transcripts/MeetingTranscript.rtf --pdf
```

End-to-end: ingest → cleanup → prompts you in the terminal for speaker names → extract → format → ejects models from VRAM. Output lands in `output/final_summaries/` — always as `.md`, additionally as `.pdf` when `--pdf` is passed.
...
# Also write a PDF alongside the markdown summary (opt-in, off by default)
uv run main.py transcripts/MeetingTranscript.rtf --pdf
```

> [!Note]
> `--pdf` is opt-in. Default behaviour is unchanged — only `<stem>_summary.md` is written. With the flag, an additional `<stem>_summary.pdf` is generated by `pipeline/pdf_export.py` using [`markdown-pdf`](https://pypi.org/project/markdown-pdf/) (pure Python, no system libs). The markdown success signal is independent of the PDF: if PDF generation fails for any reason, the pipeline still exits 0 and the `.md` is still available — you'll just see a warning line.
...
flowchart TD
    ...
    PROG --> OUT[Rendered summary<br/>+ Copy + Download .md / .pdf<br/>+ Rendered/Raw/PDF toggle]
...
6. **Final summary** rendered inline, with a three-state **Rendered / Raw / PDF** toggle. **Copy** puts the markdown source on the clipboard (not the rendered HTML). The **Download** button is smart: in Rendered or Raw mode it downloads the `.md`; in PDF mode it downloads the `.pdf`. The PDF is generated lazily — the first time you click **PDF** you'll see a "Generating PDF…" placeholder for 1–3 seconds while conversion runs, then an inline iframe of the PDF appears. Subsequent clicks on PDF within the same session serve the cached file instantly.
